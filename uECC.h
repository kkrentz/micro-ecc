...
Inputs:
    public_key_reconstruction_data - 2 * curve size bytes to restore the public key.
...
Outputs:
    public_key_reconstruction_data  - 2 * curve size bytes to restore the public key.
    private_key_reconstruction_data - curve size bytes to restore the private key.
...
public_key should be ensured to match with the one restored by uECC_restore_ecqv_public_key().
...
/* uECC_restore_ecqv_public_key() function.
Restore an ECQV public key.
...
Returns 1 if the ECQV public key was restored successfully, 0 if an error occurred.
 */
